...
import { getInsuranceDocumentFile } from "@/lib/insurance-docs";
...
export async function GET(request: NextRequest) {
  ...
  if (!allowed.includes("cockpit_versicherungen")) return new Response("Kein Zugriff", { status: 403 });

  const id = Number(request.nextUrl.searchParams.get("id"));
  if (!Number.isFinite(id) || id <= 0) return new Response("Ungültige ID", { status: 400 });

  const file = await getInsuranceDocumentFile(id);
