...
export async function findStatementImport(fileHash: string): Promise<StatementImport | null> {
  const [rows] = await getPool().query<Row[]>(
    `SELECT si.file_hash, si.filename, si.statement_number, si.tx_count, si.total, si.imported_at,
...
export async function recordStatementImport(input: {
  ...
  await getPool().query(
    `INSERT INTO bank_statement_imports (file_hash, filename, statement_number, tx_count, total, imported_by)
     VALUES (?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE filename = VALUES(filename), statement_number = VALUES(statement_number),
       tx_count = VALUES(tx_count), total = VALUES(total), imported_by = VALUES(imported_by), imported_at = NOW()`,
    [input.fileHash, input.filename, input.statementNumber, input.txCount, input.total, input.userId]
  );
}
